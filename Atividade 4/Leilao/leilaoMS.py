...
def criar_leilao():
    ...
    if request.is_json:
        ...
        if "desc" not in novo_leilao or "data_inic" not in novo_leilao or "data_fim" not in novo_leilao:
        ...
        schedule.every(espera_inic).seconds.do(leilao_agendado, evento=INICIAR, lei_id=novo_leilao['lei_id'], channel=channel)
        print(f"inicio de {novo_leilao['lei_id']} agendado para daqui {espera_inic}s")
        schedule.every(espera_fim).seconds.do(leilao_agendado, evento=FINALIZAR, lei_id=novo_leilao['lei_id'], channel=channel)
